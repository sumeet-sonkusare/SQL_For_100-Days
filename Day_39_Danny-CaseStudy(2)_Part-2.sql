...
--What was the average time in minutes it took for each runner to arrive at the Pizza Runner HQ to pickup the order?
 ...
 SELECT runner_id, 
 AVG(EXTRACT(EPOCH FROM pickup_time) - EXTRACT(EPOCH FROM order_time))/60
...
--What was the average distance travelled for each customer?
 ...
 SELECT  r.runner_id, c.customer_id, SUM(distance) AS Total_Distance
 FROM pizza_runner.customer_orders c
 JOIN pizza_runner.runner_orders r
 ON c.order_id = r.order_id
...
| runner_id | customer_id | total_distance |
| --------- | ----------- | -------------- |
| 1         | 101         | 40             |
| 1         | 102         | 26.8           |
| 1         | 104         | 20             |
...
--What was the average speed for each runner for each delivery and do you notice any trend for these values?
    ...
    SELECT runner_id,AVG(speed) AS speed_kmph
    FROM (SELECT *,(distance/duration)*60 AS Speed
    FROM pizza_runner.runner_orders) t1 
    GROUP BY runner_id;
